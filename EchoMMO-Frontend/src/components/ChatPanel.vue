<template>
  <div class="chat-panel">
    <div class="chat-header">
      <span class="header-title"><i class="fas fa-globe-asia"></i> Kênh Thế Giới</span>
    ...
    <div class="chat-messages custom-scroll" ref="chatContainer">
      <div v-if="chatStore.isLoading" class="system-msg">Đang tải thiên thư...</div>
      <div v-else-if="messages.length === 0" class="system-msg">Chưa có ai đàm đạo.</div>
      ...
      <div v-for="(msg, index) in messages" :key="index" class="chat-row">
        ...
        <span class="sender" :class="getRoleClass(msg.role)" @click="handleUserClick(msg)">
          {{ msg.senderName }}:
        </span>
...
.chat-panel {
  ...
  background: rgba(0, 0, 0, 0.6); border-radius: 4px; overflow: hidden;
  font-family: "Roboto", sans-serif; font-size: 0.9em; border: 1px solid #5d4037;
}
.chat-header {
  background: #2b1d1a; padding: 6px 10px; color: #fbc02d; font-weight: bold;
  display: flex; justify-content: space-between; align-items: center;
  border-bottom: 1px solid #5d4037; font-size: 0.85em;
}
.connection-status { display: flex; align-items: center; gap: 5px; }
.status-text { font-size: 0.8em; font-weight: normal; }
...
.status-dot.online { background: #00e676; box-shadow: 0 0 5px #00e676; }

.chat-messages {
  flex: 1; overflow-y: auto; padding: 8px; display: flex; flex-direction: column; gap: 4px; background: rgba(0,0,0,0.2);
}
.system-msg { color: #aaa; text-align: center; margin-top: 10px; font-style: italic; font-size: 0.9em; }

.chat-row { line-height: 1.4; word-wrap: break-word; font-size: 0.95em; }
...
.sender { font-weight: bold; margin-right: 5px; cursor: pointer; transition: 0.2s; }
.sender:hover { text-decoration: underline; filter: brightness(1.2); }
...
.chat-input { display: flex; border-top: 1px solid #5d4037; height: 36px; }
.chat-input input {
  flex: 1; background: rgba(0,0,0,0.3); border: none; color: white; padding: 0 10px; outline: none; font-size: 0.9em;
}
.chat-input input:focus { background: rgba(0,0,0,0.5); }
.chat-input input:disabled { background: rgba(0,0,0,0.1); cursor: not-allowed; }
.chat-input button {
  background: #3e2723; border: none; color: #fbc02d; padding: 0 15px; cursor: pointer; transition: 0.2s; border-left: 1px solid #5d4037;
}
