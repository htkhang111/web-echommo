...
<template>
  ...
  <CaptchaModal v-if="showCaptcha" :is-visible="showCaptcha" @success="onCaptchaSuccess" @close="showCaptcha = false" />

  <transition name="toast-slide">
    <div v-if="notificationStore.toast.visible" class="global-toast" :class="notificationStore.toast.type">
      <div class="toast-icon">
        <i v-if="notificationStore.toast.type === 'success'" class="fas fa-check-circle"></i>
        <i v-else-if="notificationStore.toast.type === 'error'" class="fas fa-times-circle"></i>
        <i v-else class="fas fa-info-circle"></i>
      </div>
      <div class="toast-content">
        <div class="toast-title">
...
import { useCharacterStore } from "./stores/characterStore"; // [ADD]
import MainLayout from "./layouts/MainLayout.vue";
import CaptchaModal from "./components/CaptchaModal.vue";
...
const authStore = useAuthStore();
const notificationStore = useNotificationStore();
const charStore = useCharacterStore(); // [ADD]
const showCaptcha = ref(false);
...
onMounted(async () => {
  ...
  // [FIX] Kiểm tra nếu đã login thì tải cả Profile lẫn Character ngay lập tức
  if (authStore.isAuthenticated) {
    try {
      await Promise.all([
        authStore.fetchProfile(),
        charStore.fetchCharacter() // [IMPORTANT] Fetch Character ngay khi App mount
      ]);
    } catch (e) {
