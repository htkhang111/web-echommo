...
import { useLeaderboardStore } from "@/stores/leaderboardStore"; // Added
import { useRouter } from "vue-router";
// [FIX] Import mới
import { getEnemyImage, getCurrentSkin, resolveItemImage } from "@/utils/assetHelper";

const battleStore = useBattleStore();
const charStore = useCharacterStore();
const authStore = useAuthStore();
const lbStore = useLeaderboardStore(); // Added
...
onMounted(async () => {
  ...
  lbStore.fetchTopMonsters(); // Fetch leaderboard info
...
.fighter-img { width: 100%; height: 100%; object-fit: contain; image-rendering: pixelated; }
.hit-anim .fighter-img { filter: brightness(2) sepia(1) hue-rotate(-50deg); transform: translateX(-5px); }

.damage-text {
  position: absolute; top: -20px; left: 50%; transform: translateX(-50%);
  font-size: 1.5em; font-weight: bold; color: #ffeb3b;
  text-shadow: 2px 2px 0 #b71c1c;
  animation: floatUp 0.5s ease-out forwards;
}
.player-dmg { color: #ff5252; }

.fighter-stats { width: 100%; margin-top: 10px; text-align: center; }
.name-tag { font-size: 0.8em; font-weight: bold; padding: 2px 8px; border-radius: 4px; display: inline-block; margin-bottom: 4px; }
.red-tag { background: #b71c1c; color: white; }
.blue-tag { background: #1565c0; color: white; }

.stat-bar-box {
  width: 100%; height: 14px; background: #333; border-radius: 2px;
  position: relative; margin-bottom: 2px; border: 1px solid #000;
}
...
.bar-text {
  ...
  font-size: 0.65em; font-weight: bold; text-shadow: 1px 1px 0 #000; z-index: 2;
}

.combat-controls { position: absolute; bottom: 20px; width: 100%; text-align: center; }
.ongoing-actions { display: flex; justify-content: center; gap: 15px; align-items: center; }
.auto-fight-btn {
  background: #333; color: #aaa; padding: 8px 15px; border-radius: 20px;
  font-size: 0.8em; font-weight: bold; display: flex; gap: 5px; align-items: center;
}
.spinner {
  width: 12px; height: 12px; border: 2px solid #aaa; border-top-color: transparent;
  border-radius: 50%; animation: spin 1s infinite linear;
}
.btn-skill {
  background: var(--gold); color: #000; font-weight: bold; border: none;
  padding: 10px 20px; border-radius: 4px; cursor: pointer;
  box-shadow: 0 0 10px rgba(253, 216, 53, 0.3); transition: 0.2s;
}
.btn-skill:disabled { background: #555; color: #888; cursor: not-allowed; box-shadow: none; }

.qte-overlay { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); z-index: 100; }
.qte-button {
  font-size: 2rem; padding: 20px 40px; background: #e74c3c; color: white;
  border: 4px solid #c0392b; animation: pulse 0.5s infinite; cursor: pointer;
}

.result-text { font-size: 2em; font-weight: 900; margin-bottom: 10px; text-shadow: 0 0 10px black; }
...
.forest-btn { background: #2e7d32; border-color: #1b5e20; box-shadow: 0 2px 5px rgba(0, 0, 0, 0.3); }
.forest-btn:hover { background: #388e3c; }

.chat-section {
  flex: 1; display: flex; flex-direction: column; background: rgba(30, 20, 10, 0.9);
  border-color: #d84315; min-height: 150px;
}
.chat-header { background: #d84315; color: white; padding: 5px 10px; font-weight: bold; font-size: 0.8em; }
.chat-messages { flex: 1; padding: 10px; font-size: 0.85em; overflow-y: auto; display: flex; flex-direction: column; gap: 4px; }

.char-detail-card { padding: 10px; margin-bottom: 15px; }
.detail-row { display: flex; justify-content: space-between; padding: 5px 0; border-bottom: 1px dashed #333; font-size: 0.9em; }
.detail-row .val { color: var(--gold); font-weight: bold; }

.combat-log-panel { flex: 1; display: flex; flex-direction: column; }
.logs-container { flex: 1; padding: 10px; overflow-y: auto; font-size: 0.8em; font-family: monospace; }
...
.loading-arena { flex: 2; display: flex; justify-content: center; align-items: center; flex-direction: column; color: #aaa; font-size: 1.2em; }
.error-box { text-align: center; }
.text-red { color: #ff5252; }
.loot-display { margin-bottom: 10px; font-weight: bold; color: gold; }

@media (max-width: 900px) {
  .battle-layout { grid-template-columns: 1fr; grid-template-rows: auto auto auto auto; height: auto; padding-bottom: 60px; }
  .center-panel { order: 1; }
  .right-panel { order: 2; }
  .left-panel { order: 3; display: none; }
  .combat-arena { height: 400px; }
  .chat-section { height: 200px; }
