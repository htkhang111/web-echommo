<!-- <template>
  <div class="page-container battle-page">
    <div class="battle-bg-layer" :style="{ backgroundImage: `url(${bgImage})` }"></div>
      ...
      <main class="center-panel">
          ...
          <div class="combat-controls">
            ...
            <div v-else class="result-actions">
              ...
              <div class="btn-group">
                ...
                <button class="btn-nav" @click="$router.push('/')">
                  🏠 Về Làng
                </button>
              </div>
            </div>
        ...
        <div class="chat-section">
          ...
          <div class="chat-messages custom-scroll">
             <div style="padding:10px; color:#aaa; font-style:italic;">Hệ thống chat đang kết nối...</div>
          </div>
        </div>
      </main>
...
// Assets
const mapId = route.query.map || 'forest';
const bgImage = getAssetUrl(`background/b_${mapId}.jpg`);

const playerImage = computed(() => {
    const skinId = authStore.user?.skinId || 'skin_yasou';
    const skin = getCurrentSkin(skinId);
...
onMounted(() => {
  lbStore.fetchTopMonsters();
  // Init battle logic here if needed
});
</script>
...
.leaderboard-list { flex: 1; padding: 10px; }
.rank-item {
  display: flex;
  align-items: center;
  gap: 10px;
...
.rank-item:hover { background: #333; }
.rank-num {
  font-weight: bold;
  color: #777;
  width: 20px;
...
.top-1 { color: #ffd700; font-size: 1.2rem; }
.top-2 { color: #c0c0c0; font-size: 1.1rem; }
.top-3 { color: #cd7f32; font-size: 1.1rem; }

.rank-avatar img { width: 32px; height: 32px; border-radius: 50%; border: 1px solid #555; }
.rank-info { flex: 1; overflow: hidden; }
.rank-name { font-size: 0.9rem; font-weight: bold; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.rank-power { font-size: 0.8rem; color: #ef5350; }
...
.hit-anim { animation: shake 0.3s; filter: brightness(2) sepia(1) hue-rotate(-50deg) saturate(5); }
...
.fighter-img { width: 100%; height: 100%; object-fit: cover; }

.damage-text {
  position: absolute; top: -20px; left: 50%;
  transform: translateX(-50%);
  color: #ff5252; font-weight: 900; font-size: 1.5rem;
...
.name-tag {
  padding: 4px 10px; border-radius: 4px; font-weight: bold; font-size: 0.9rem; text-align: center;
  margin-bottom: 5px; width: 100%; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
}
.red-tag { background: var(--red-enemy); color: white; }
.blue-tag { background: var(--blue-player); color: white; }

.stat-bar-box { width: 100%; height: 12px; background: #333; border-radius: 6px; overflow: hidden; position: relative; margin-top: 4px; border: 1px solid #000; }
.bar-fill { height: 100%; transition: width 0.3s ease; }
.hp-fill { background: #d32f2f; }
.energy-fill { background: #fbc02d; }
.bar-text {
  position: absolute; top: 0; left: 0; width: 100%; height: 100%;
  display: flex; align-items: center; justify-content: center;
  font-size: 0.65rem; color: white; font-weight: bold; text-shadow: 0 0 2px black;
}
...
.ongoing-actions { display: flex; gap: 20px; align-items: center; }
.auto-fight-btn {
  background: #333; padding: 8px 15px; border-radius: 20px;
  border: 1px solid #555; font-size: 0.8rem; color: #aaa;
  display: flex; gap: 8px; align-items: center;
}
.spinner { width: 12px; height: 12px; border: 2px solid #555; border-top-color: #fff; border-radius: 50%; animation: spin 1s infinite linear; }
@keyframes spin { to { transform: rotate(360deg); } }

.btn-skill {
  background: linear-gradient(45deg, #1565c0, #0d47a1);
  border: none; padding: 10px 20px; color: white; font-weight: bold;
  border-radius: 6px; cursor: pointer;
  box-shadow: 0 4px 0 #002171; transition: 0.1s;
}
...
.result-actions { text-align: center; background: rgba(0,0,0,0.8); padding: 15px 30px; border-radius: 10px; border: 1px solid #555; }
.result-text { font-size: 1.5rem; font-weight: 900; margin-bottom: 10px; }
.VICTORY { color: #4caf50; }
.DEFEAT { color: #f44336; }
.loot-display { color: #ffd700; margin-bottom: 10px; font-weight: bold; }

.btn-group { display: flex; gap: 10px; justify-content: center; }
.btn-nav { padding: 8px 15px; background: #424242; border: 1px solid #616161; color: white; cursor: pointer; border-radius: 4px; }
.forest-btn { background: #2e7d32; border-color: #43a047; }
...
.chat-header { padding: 8px; background: #222; font-weight: bold; font-size: 0.8rem; border-bottom: 1px solid #444; color: #aaa; }
.chat-messages { flex: 1; padding: 10px; font-size: 0.9rem; }

.right-panel { display: flex; flex-direction: column; gap: 10px; }
.char-detail-card { padding: 10px; background: rgba(0,0,0,0.3); border-radius: 6px; }
.detail-row { display: flex; justify-content: space-between; padding: 4px 0; border-bottom: 1px dashed #444; font-size: 0.85rem; }
.detail-row:last-child { border-bottom: none; }
.detail-row i { width: 20px; text-align: center; color: #aaa; }
.val { font-weight: bold; color: #fff; }

.combat-log-panel { flex: 1; display: flex; flex-direction: column; overflow: hidden; }
.logs-container { flex: 1; padding: 10px; font-family: monospace; font-size: 0.85rem; line-height: 1.4; }
.log-entry { margin-bottom: 4px; padding-left: 8px; border-left: 2px solid transparent; }
.log-player { border-left-color: #4caf50; color: #a5d6a7; }
.log-enemy { border-left-color: #ef5350; color: #ef9a9a; }
.log-win { color: #ffd700; font-weight: bold; border-left-color: #ffd700; background: rgba(255, 215, 0, 0.1); }

@media (max-width: 900px) {
  .battle-layout { grid-template-columns: 1fr; height: auto; }
  .side-panel { display: none; }
  .combat-arena { min-height: 400px; }
}
</style> -->
      ...
      <aside class="side-panel left-panel">
        ...
        <div class="leaderboard-list custom-scroll">
          ...
          <div
            ...
            :key="index"
          >
            <div class="rank-num" :class="'top-' + (index + 1)">
              {{ index + 1 }}
            </div>
          ...
          <div
            v-if="!lbStore.loadingMonsters && lbStore.topMonsters.length === 0"
            class="empty-mini"
          >
            Chưa có dữ liệu
          </div>
          ...
          <div class="combat-arena" v-if="battleStore.enemy">
            ...
            <div class="fighter-card enemy" :class="{ 'hit-anim': isEnemyHit }">
              <div class="fighter-visual">
                ...
                <div class="damage-text" v-if="isEnemyHit">
                  -{{ lastDamage }}
                </div>
              </div>
              <div class="fighter-stats">
            ...
            <div class="combat-controls">
              ...
              <div v-else class="result-overlay">
                <div class="result-content">
                  ...
                  <div
                    v-if="battleStore.status === 'VICTORY'"
                    class="rewards-row"
                  >
                    <div class="reward-item">
                      <span class="icon">💰</span>
                      <span class="val"
                        >+{{ battleStore.rewardGold || 0 }} Vàng</span
                      >
                    </div>
                    <div class="reward-item">
                      <span class="icon">✨</span>
                      <span class="val"
                        >+{{ battleStore.rewardExp || 0 }} EXP</span
                      >
                    </div>
                  </div>
                  ...
                  <div class="btn-group-large">
                    <button
                      class="btn-big forest"
                      @click="$router.push('/explore')"
                    >
                      <i class="fas fa-tree"></i> Về Rừng
                    </button>
                    <button
                      class="btn-big village"
...
const triggerFireworks = () => {
  ...
  setTimeout(
    () => createExplosion((canvas.width * 2) / 3, canvas.height / 3),
    400,
  );
  ...
  (st) => {
    if (st === "VICTORY") {
      ...
      if (item) {
        ...
        if (
          rareTypes.includes(item.rarity) ||
          rareTypes.includes(item.baseRarity)
        ) {
...
onMounted(async () => {
  ...
  // [FIX] Sửa tên hàm gọi API lấy bảng xếp hạng quái
  if (lbStore.fetchMonsterBoard) {
...
.btn-skill {
  ...
  cursor: pointer;
  box-shadow:
    0 5px 15px rgba(253, 216, 53, 0.4),
...
.qte-button {
  ...
  letter-spacing: 2px;
  animation: pulseQTE 0.6s infinite;
  cursor: pointer;
...
.result-title.DEFEAT {
  color: #b0bec5;
  text-shadow: 0 0 20px rgba(176, 190, 197, 0.5);
}
