...
export const useInventoryStore = defineStore("inventory", {
  ...
  actions: {
    async fetchInventory() {
      this.isLoading = true;
      try {
        // [FIX] Đổi path từ /equipment/inventory -> /inventory/items
    ...
    async equipItem(id) {
      ...
      try {
        // [FIX] /equipment/equip -> /inventory/equip
    ...
    async unequipItem(id) {
      const charStore = useCharacterStore();
      try {
        // [FIX] /equipment/unequip -> /inventory/unequip
    ...
    async useItem(id) {
      const charStore = useCharacterStore();
      try {
        // [FIX] /equipment/use -> /inventory/use
