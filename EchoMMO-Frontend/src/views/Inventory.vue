...
const createVirtualItem = (id, name, img, qty) => ({
  ...
  item: {
    ...
    type: "MATERIAL",
    rarity: "COMMON",
    imageUrl: img, // assetHelper sẽ tự map
    description: "Tài nguyên khai thác được.",
    basePrice: 0,
  },
});

// --- FILTERED ITEMS (CORE FIX) ---
const filteredItems = computed(() => {
  // 1. Debug log để xem dữ liệu (F12)
  console.log("Inventory Items:", inventoryStore.items);
  console.log("Wallet Data:", authStore.wallet);

  let items = [];

  // 2. Lấy item từ Inventory Store
  if (inventoryStore.items && Array.isArray(inventoryStore.items)) {
    items = [...inventoryStore.items];
  }

  // 3. Lấy item từ Wallet Store (Tài nguyên)
  if (authStore.wallet) {
    const w = authStore.wallet;
    
    // Support cả 2 kiểu đặt tên (camelCase và snake_case)
    ...
    if (woodVal > 0) items.push(createVirtualItem("v_wood", "Gỗ Sồi", "r_wood.png", woodVal));
    if (stoneVal > 0) items.push(createVirtualItem("v_stone", "Đá", "stone_1.png", stoneVal));
    if (ironVal > 0) items.push(createVirtualItem("v_iron", "Quặng Sắt", "r_iron_ore.png", ironVal));
    if (platinumVal > 0) items.push(createVirtualItem("v_plat", "Bạch Kim", "r_platinum.png", platinumVal));
...
onMounted(async () => {
  // Gọi đồng thời để tối ưu tốc độ và đảm bảo có data
  await Promise.all([
    inventoryStore.fetchInventory(),
    charStore.fetchCharacter(),
