        ...
        <div class="inventory-grid custom-scroll">
          <div v-for="u in filteredItems" :key="u.userItemId" class="grid-slot" :class="[
            ...
            <div class="slot-inner">
              <img :src="getItemImage(getSafeItem(u).image || getSafeItem(u).imageUrl)" class="item-icon" alt="item"
              ...
              <div v-if="u.enhanceLevel > 0" class="level-tag-mini" :class="getLevelClass(u.enhanceLevel)">
                +{{ u.enhanceLevel }}
              </div>
            </div>
          ...
          <div v-for="n in Math.max(0, 50 - filteredItems.length)" :key="'empty-' + n" class="grid-slot empty"></div>
        </div>
      </div>

      <div class="right-panel-container">
        <transition name="fade-slide" mode="out-in">
          <div v-if="selectedItem" class="item-card-panel" :key="selectedItem.userItemId">
            <div class="card-header-bg">
              <div class="card-icon-wrapper">
                <div class="card-icon-frame"
                  :class="'rarity-' + (selectedItem.rarity || getSafeItem(selectedItem).rarity || 'COMMON')">
                  <img :src="getItemImage(getSafeItem(selectedItem).image || getSafeItem(selectedItem).imageUrl)"
                    @error="$event.target.src = getItemImage('o_coalOre.png')" />
                </div>
            ...
            <div class="card-actions">
              <button v-if="getSafeItem(selectedItem).type === 'CONSUMABLE' && !selectedItem.isVirtual"
                class="card-btn use-btn" @click="inventoryStore.useItem(selectedItem.userItemId)">
                DÙNG
              </button>

              <template v-if="canEquip(selectedItem)">
                <button v-if="!selectedItem.isEquipped" class="card-btn equip-btn"
                  @click="inventoryStore.equipItem(selectedItem.userItemId)">
                  TRANG BỊ
                </button>
                <button v-else class="card-btn unequip-btn"
                  @click="inventoryStore.unequipItem(selectedItem.userItemId)">
                  GỠ BỎ
                </button>
              </template>

              <button v-if="!selectedItem.isEquipped && !selectedItem.isVirtual" class="card-btn sell-btn"
...
const filteredItems = computed(() => {
  ...
  if (authStore.wallet) {
    ...
    if (stoneVal > 0) items.push(createVirtualItem("v_stone", "Đá", "o_coalOre", stoneVal));
...
onMounted(async () => {
  updateDayNight(); // Kích hoạt đổi màu nền
  await Promise.all([
    inventoryStore.fetchInventory(),
    charStore.fetchCharacter(),
    authStore.fetchProfile(),
...
.inventory-page {
  /* Xóa màu nền cũ, để trong suốt cho bg-layer hiển thị */
...
/* --- 2. LAYOUT HÀNH TRANG (Z-INDEX CAO HƠN NỀN) --- */
.inventory-wrapper {
  position: relative;
  z-index: 10;
  height: 100vh;
...
.main-chest-frame {
  width: 600px;
  height: 85vh;
  background: rgba(20, 20, 25, 0.95); /* Giữ nền tối để nhìn rõ item */
  border: 1px solid #444;
  box-shadow: 0 0 50px rgba(0, 0, 0, 0.8);
  display: flex;
  flex-direction: column;
...
.item-card-placeholder {
  ...
  color: #555;
  font-style: italic;
  /* Đảm bảo chữ dễ đọc trên nền mới */
