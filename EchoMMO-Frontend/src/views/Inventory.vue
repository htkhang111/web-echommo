...
const onMouseDown = (e) => {
    isDragging = true; startY = e.clientY; lastTouchY = startY; velocity = 0; targetScrollY = null; cancelAnimationFrame(animationFrameId);
...
const onMouseUp = (e) => {
    ...
    velocity = (startY - e.clientY) * 0.05; lastTime = 0; inertiaLoop(performance.now());
};
...
const getStatName = (code) => {
    const dict = { "ATK": "Công Lực", "ATK_FLAT": "Công Lực", "ATK_PERCENT": "Công Lực", "DEF": "Hộ Thể", "DEF_FLAT": "Hộ Thể", "DEF_PERCENT": "Hộ Thể", "HP": "Sinh Lực", "HP_FLAT": "Sinh Lực", "HP_PERCENT": "Sinh Lực", "SPEED": "Thân Pháp", "CRIT": "Bạo Kích", "CRIT_RATE": "Bạo Kích", "CRIT_DMG": "Sát Thương Bạo", "LUCK": "May Mắn", "DURABILITY": "Độ Bền" };
    return dict[code] || code;
};
const getStatLabel = (mainStatType, itemType) => {
    if (mainStatType && typeof mainStatType === 'string') return getStatName(mainStatType);
    if (itemType) { if (itemType === 'WEAPON') return "Công Lực"; if (['ARMOR', 'HELMET', 'BOOTS', 'RING', 'NECKLACE'].includes(itemType)) return "Hộ Thể"; if (itemType === 'TOOL') return "Hiệu Suất"; }
    return "Sức Mạnh";
};
...
/* Header */
.panel-header {
  background: linear-gradient(180deg, #3e2723 0%, #261815 100%);
  padding: 10px 16px;
...
.btn-add-slots, .btn-icon-action {
    border: 1px solid #5d4037; color: #ffd700; width: 26px; height: 26px; border-radius: 4px;
...
.filter-tabs button {
    ...
    font-size: 0.8rem; border-radius: 4px; transition: 0.2s; font-family: 'Noto Serif TC', serif;
}
...
.center-highlight-bar {
    ...
    background: linear-gradient(90deg, transparent 0%, rgba(255, 215, 0, 0.1) 20%, rgba(255, 215, 0, 0.1) 80%, transparent 100%);
...
.wheel-item {
    ...
    will-change: transform, opacity, filter; cursor: pointer; transition: filter 0.2s;
}
.wheel-inner {
    width: 90%; height: 44px; display: flex; align-items: center;
    background: rgba(38, 24, 21, 0.8); border: 1px solid #4e342e; border-radius: 4px;
    padding: 0 10px; transition: background 0.2s, border 0.2s;
}
...
.item-slot {
  aspect-ratio: 1; border: 1px solid #4e342e; background: rgba(0, 0, 0, 0.4);
...
.detail-image-box { 
    ...
    margin-bottom: 15px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.05);
}
...
.btn-action {
    width: 100%; padding: 12px; margin-top: 8px; border: none; font-weight: bold; cursor: pointer; text-transform: uppercase; letter-spacing: 1px;
    clip-path: polygon(10px 0, 100% 0, 100% calc(100% - 10px), calc(100% - 10px) 100%, 0 100%, 0 10px);
    transition: 0.2s; font-family: 'Noto Serif TC', serif;
}
