...
const startTimer = () => {
  ...
  timerInterval = setInterval(() => {
    ...
    if (countdown.value <= 0) {
      ...
      finishRest(); // Hết giờ -> Gọi hoàn thành
    }
  }, 1000);
};
...
const finishRest = async () => {
  try {
    ...
    console.error(e);
    isResting.value = false; // Thoát trạng thái treo nếu lỗi
  }
};
...
<style scoped>
/* Copy Style cũ + Thêm Style cho Menu Spa */
...
const closeRestModal = () => {
  showRestModal.value = false;
  // Xóa sạch storage khi người dùng xác nhận xong
...
const confirmRest = async (type) => {
  ...
  try {
    ...
    // Lấy thời gian duration từ server (hoặc mặc định)
    const duration = res.data.secondsRemaining || (type === "VIP" ? 10 : 60);

    // Tính thời điểm kết thúc tương lai (Unix timestamp)
    const finishTime = Date.now() + duration * 1000;

    // Lưu vào LocalStorage để duy trì khi F5
    localStorage.setItem(STORAGE_KEY_END_TIME, finishTime);

    countdown.value = duration;
    isResting.value = true;
    await authStore.fetchProfile(); // Update tiền trong ví
...
const startTimer = () => {
  ...
  timerInterval = setInterval(() => {
    // Luôn tính lại thời gian còn lại dựa trên mốc kết thúc đã lưu
...
const finishRest = async () => {
  try {
    ...
    // Xóa storage vì đã hoàn thành
    localStorage.removeItem(STORAGE_KEY_END_TIME);
  } catch (e) {
    console.error(e);
    // Nếu lỗi không phải do đã hoàn thành trước đó, thì reset trạng thái
...
// --- LOGIC 3: KHÔI PHỤC KHI F5 / MỞ LẠI TAB ---
const checkBackgroundRest = () => {
  ...
  if (savedEndTime) {
    ...
    if (remaining > 0) {
      // Vẫn còn thời gian -> Tiếp tục đếm
      isResting.value = true;
      countdown.value = remaining;
      startTimer();
    } else {
      // Đã hết giờ trong lúc offline -> Gọi finish ngay lập tức
      isResting.value = true;
      finishRest();
    }
  }
