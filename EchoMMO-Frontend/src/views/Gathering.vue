...
const handleGather = async (times) => {
  ...
  // [FIX] Kiểm tra xem đã có thông tin nhân vật chưa
  ...
  try {
    // [FIX] Lấy ID nhân vật (hỗ trợ cả _id cho MongoDB và id cho SQL)
    ...
    const res = await axiosClient.post("/exploration/gather", {
      characterId: charId,   // <--- QUAN TRỌNG: Thêm trường này để fix lỗi 400
    ...
    console.error(error);
    // Hiển thị lỗi từ backend nếu có message
    feedbackMsg.value = error.response?.data?.message || error.response?.data || "Lỗi khai thác!";
  } finally {
...
onMounted(() => {
  ...
  authStore.fetchProfile(); // Load ví để đồng bộ
  initEvent();
});
</script>
...
const handleGather = async (times) => {
  ...
  if ((charStore.character?.energy || 0) < times) { feedbackMsg.value = "⚠️ Nội năng không đủ!"; return; }
  ...
  try {
    const charId = charStore.character._id || charStore.character.id || charStore.character.charId;
    const res = await axiosClient.post("/exploration/gather", {
      characterId: charId,
    ...
    remainingNode.value -= actualGathered;
    if (charStore.character) charStore.character.energy = res.data.currentEnergy;
    if (authStore.wallet) { authStore.wallet.wood = res.data.wood; authStore.wallet.stone = res.data.stone; }
...
const handleGatherAll = () => {
  const times = Math.min(10, remainingNode.value, charStore.character?.energy || 0);
  if (times > 0) handleGather(times); else feedbackMsg.value = "⚠️ Nội năng cạn kiệt!";
};
...
onMounted(() => { charStore.fetchCharacter(); authStore.fetchProfile(); initEvent(); });
</script>
