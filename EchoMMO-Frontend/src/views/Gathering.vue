...
// [FIX] Sửa 'lv' thành 'level' để khớp với Character.java Backend
...
const initEvent = () => {
  ...
  if (evt) {
    currentEvent.value = evt;
    // Backend default is 0, so if 0, it means empty node.
    remainingNode.value = dbAmount !== undefined ? dbAmount : 0;
    maxNode.value = 10; 
  } else {
    currentEvent.value = EVENT_TYPES[0]; 
  }
...
const handleGather = async (times = 1) => {
  ...
  try {
    ...
    // [FIX] Lấy charId đúng từ Entity Backend
    const char = charStore.character;
    const charId = char.charId; 

    if (!charId) {
      throw new Error("Không tìm thấy ID nhân vật (char.charId bị null)");
    }
