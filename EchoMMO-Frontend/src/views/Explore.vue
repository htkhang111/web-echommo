...
const handleResult = async () => {
  ...
  try {
    ...
    // 3. Gặp Quái (Enemy)
      ...
      targetImage.value = new URL(`../assets/enemy/${enemyImgName}`, import.meta.url).href;
          ...
          <div class="status-header">
            ...
            <div class="bars-container">
              <div class="stat-group">
                <div class="stat-row">
                  <span class="stat-icon">❤️</span>
                  <div class="progress-bg"><div class="progress-fill hp" :style="{ width: charStore.hpPercent + '%' }"></div></div>
                </div>
                <div class="stat-row">
                  <span class="stat-icon">⚡</span>
                  <div class="progress-bg"><div class="progress-fill energy" :style="{ width: charStore.energyPercent + '%' }"></div></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="action-panel">
            <template v-if="!isEncounter">
              ...
              <button class="btn-action main-btn" @click="startExploration" :disabled="isMoving">
                <span v-if="!isMoving"><i class="fas fa-walking"></i> HÀNH TẨU</span>
                <span v-else>... ({{ countdown }}s)</span>
              </button>
      ...
      <div class="map-modal-card">
        ...
        <div class="map-grid">
          <div v-for="map in maps" :key="map.id" class="map-item" :class="{ active: currentMapId === map.id, locked: userLv < map.minLv }" @click="selectMap(map)">
            <div class="map-info"><div class="map-name">{{ map.name }}</div><div class="map-lv">Lv.{{ map.minLv }}-{{ map.maxLv }}</div></div>
            <div v-if="userLv < map.minLv" class="lock-icon">🔒</div>
          </div>
        </div>
        <button class="close-btn" @click="showMapModal = false">Đóng</button>
...
import ChatPanel from "@/components/ChatPanel.vue";
import QuestPanel from "@/components/QuestPanel.vue";
import { getItemImage, getCurrentSkin, getAssetUrl, getEnemyImage } from "@/utils/assetHelper"; // [FIX]
...
const selectMap = (map) => {
  if (userLv.value < map.minLv) { addLog(`🔒 Cần Lv.${map.minLv}`); return; }
  currentMapId.value = map.id;
  showMapModal.value = false;
  addLog(`Đã chọn: <b>${map.name}</b>`);
};

// [FIX] Lấy background online
...
const handleResult = async () => {
  ...
  const eventChance = Math.random() * 100;
  if (eventChance < 20) {
    addLog(`<span style="color:#00e676; font-weight:bold;">🌿 Phát hiện khu vực tài nguyên!</span>`);
...
<style scoped>
.explore-page { padding: 10px; height: 100vh; overflow: hidden; color: #eee; font-family: "Noto Serif TC", serif; background-color: #1a1a1a; }
.explore-layout { display: grid; grid-template-columns: 1fr 300px; gap: 15px; height: 100%; max-width: 1400px; margin: 0 auto; }
.center-zone { display: flex; flex-direction: column; gap: 10px; height: 100%; overflow: hidden; }
.game-board { flex: 0 0 320px; background: #261815; border: 2px solid #5d4037; border-radius: 8px; display: flex; flex-direction: column; position: relative; }
.chat-board { flex: 1; min-height: 0; background: rgba(0,0,0,0.5); border: 1px solid #444; border-radius: 8px; }
...
.avatar-circle, .avatar-target { width: 96px; height: 96px; display: flex; justify-content: center; align-items: center; filter: drop-shadow(0 5px 5px rgba(0,0,0,0.5)); }
...
.main-btn { flex: 2; background: linear-gradient(to bottom, #4e342e, #3e2723); border: 1px solid #c5a059; color: #c5a059; }
.sub-btn { flex: 0.5; background: #3e2723; border: 1px solid #5d4037; }
.right-zone { display: flex; flex-direction: column; gap: 15px; height: 100%; }
.log-panel { flex: 0 0 40%; background: #1e1e1e; border: 2px solid #5d4037; border-radius: 8px; display: flex; flex-direction: column; }
.log-content { flex: 1; padding: 8px; overflow-y: auto; background: #111; font-size: 0.85em; }
.quest-panel-wrapper { flex: 1; background: #1e1e1e; border: 2px solid #5d4037; border-radius: 8px; overflow: hidden; }
...
.lock-icon { position: absolute; right: 10px; top: 10px; }
.close-btn { width: 100%; padding: 10px; background: #b71c1c; border: none; color: white; font-weight: bold; cursor: pointer; }
.encounter-modal { position: fixed; inset: 0; background: rgba(0,0,0,0.85); z-index: 2000; display: flex; align-items: center; justify-content: center; }
.modal-card { width: 350px; background: #261815; border: 2px solid #b71c1c; border-radius: 12px; }
.modal-header { background: #b71c1c; color: #fff; padding: 10px; text-align: center; font-weight: bold; }
.modal-body { padding: 20px; text-align: center; }
.modal-footer { padding: 15px; display: flex; gap: 15px; }
.modal-btn { flex: 1; padding: 10px; cursor: pointer; font-weight: bold; }
.modal-btn.flee { background: #555; color: #ccc; }
.modal-btn.fight { background: #d32f2f; color: #fff; }
@media (max-width: 900px) { .explore-layout { grid-template-columns: 1fr; } .right-zone { height: 400px; } }
</style>