...
const setLog = (msg, type = "TEXT", image = null, action = null) => {
  ...
  try {
    ...
    if (res.type === "ITEM" && res.rewardName) {
      ...
      if (questStore.checkQuestCompletion) {
        ...
        if (rGold > 0) {
            msg += `<br><span style="color:#fff; font-size:0.9em;">(Quest: +${rGold} Vàng)</span>`;
      ...
      setLog(`<span style="color:#ef5350; font-weight:bold; font-size:1.1em;">⚠️ ${res.message}</span>`, "ENEMY", img, { label: "⚔️ CHIẾN ĐẤU", handler: () => router.push("/battle") });
    } 
    else {
      setLog(`<span style="color:#eee;">${res.message}</span>`, "TEXT");
    }

  } catch (e) {
    if (e.message === "CAPTCHA") captchaModal.value.open();
    else setLog(`<span style="color:red">Lỗi: ${e.message}</span>`, "TEXT");
  }
};
...
const startExploration = () => {
  ...
  // [CHECK 1] Kiểm tra thể lực tại Client trước khi chạy
...
const handleResult = async () => {
  ...
  try {
    ...
    // [CHECK 2] Xử lý lỗi từ Server trả về (ví dụ khi hack hoặc lag)
    if (e.message === "CAPTCHA") {
        captchaModal.value.open();
    } 
    else if (e.message && (e.message.includes("thể lực") || e.message === "NOT_ENOUGH_ENERGY" || e.message.includes("Energy"))) {
