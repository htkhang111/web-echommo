  ...
  <div class="admin-page custom-scrollbar">
    ...
    <div class="admin-container">
      <header class="admin-header fade-in-down">
        <div class="header-content">
          <h1 class="glow-text">QUẢN TRỊ THIÊN PHỦ</h1>
          <p class="subtitle">Lò Luyện Khí & Điều Phối Thiên Cơ</p>
        </div>
        <div class="header-decor"></div>
      </header>
          ...
          <transition name="fade">
            <div v-if="showCreateItem" class="create-panel glass-panel fade-in wizard-container">
               ...
               <form @submit.prevent="createItem" class="wizard-form">
                  ...
                  <div v-show="currentStep === 1" class="wizard-step active">
                  ...
                  <div v-show="currentStep === 2" class="wizard-step active">
                  ...
                  <div v-show="currentStep === 3" class="wizard-step active">
                    <div class="step-label">3. Chọn Hình Dáng (Asset)</div>
                    <div class="assets-wrapper custom-scrollbar">
                       <div class="assets-grid">
                           <div v-for="(img, idx) in filteredAssets" :key="idx" 
                  ...
                  <div v-show="currentStep === 4" class="wizard-step active">
                    <div class="step-label">4. Chọn Phẩm Cấp (Rarity)</div>
                  ...
                  <div v-show="currentStep === 5" class="wizard-step active">
                    <div class="step-label">5. Thổi Hồn (Chỉ Số)</div>
                    
                    <div class="stat-section">
                      <label class="sub-label highlight">1. Chọn Chỉ Số Chính (Duy Nhất)</label>
                      <div class="stat-type-selector">
                          <div v-for="stat in mainStatOptions" :key="stat.value"
                      ...
                      <div class="input-group mt-2">
                          <input type="number" v-model.number="selectedMainStatValue" 
                                 class="big-input" placeholder="Nhập giá trị chỉ số chính..." 
                    ...
                    <div class="stat-section">
                      ...
                      <div v-else class="empty-text">
                          Phẩm chất này không sở hữu dòng phụ.
                      </div>
                    </div>
                  ...
                  <div v-show="currentStep === 6" class="wizard-step active">
                     ...
                     <label class="checkbox-container">
                        ...
                        <div class="label-content">
                          <strong>Bán Đại Trà (Shop Hệ Thống)</strong>
                          <p>Nếu tích: Vật phẩm sẽ xuất hiện trong Shop Vô Hạn.</p>
                          <p class="warning-text">Nếu không tích: Vật phẩm sẽ ẨN khỏi shop (Chỉ dùng để Drop/Event).</p>
                        </div>
                     </label>
            ...
            <table class="wuxia-table">
              ...
              <tbody>
                <tr v-for="i in filteredItems" :key="i.itemId">
                  <td class="id-col">{{ i.code }}</td>
                  <td class="name-col">
                     <img :src="i.imageUrl" class="mini-icon"/> {{ i.name }}
                  </td>
                  <td>{{ i.type }}</td>
                  <td :class="['rarity', 'rarity-' + i.rarity]">{{ i.rarity }}</td>
                  <td class="gold-val">{{ formatNumber(i.basePrice) }}</td>
                  <td>
                     <span v-if="i.isSystemItem" class="badge active">Vô Hạn</span>
                     <span v-else class="badge banned">Ẩn/Drop</span>
                  </td>
                  <td>
                    <button @click="requestDeleteItem(i)" class="btn-icon delete">
                      <i class="fas fa-trash-alt"></i>
...
// --- NEW STATE CHO MAIN STAT ---
const selectedMainStatType = ref('ATK'); // Mặc định ATK
...
const itemTypes = [
  ...
  { value: 'MATERIAL', label: 'Nguyên Liệu', icon: 'fas fa-gem' }
];
...
// Mock Assets (Thay bằng URL thật của bạn)
const mockAssets = {
    WEAPON: ['/assets/items/sword_1.png', '/assets/items/axe_1.png', '/assets/items/bow_1.png'],
    ARMOR: ['/assets/items/armor_1.png', '/assets/items/robe_1.png'],
    HELMET: ['/assets/items/helmet_1.png'],
    BOOTS: ['/assets/items/boots_1.png'],
    CONSUMABLE: ['/assets/items/potion_red.png', '/assets/items/potion_blue.png'],
    MATERIAL: ['/assets/items/wood.png', '/assets/items/iron.png']
};
...
const itemForm = reactive({
    ...
    rarity: "COMMON", basePrice: 0, imageUrl: "", isSystemItem: false,
...
const filteredAssets = computed(() => {
    return mockAssets[itemForm.type] || mockAssets['WEAPON'];
});
...
const selectType = (t) => {
    itemForm.type = t;
    itemForm.slotType = t;
    setTimeout(() => nextStep(), 200); // Auto next with delay
};
...
const getSubStatInfo = (rarity) => {
    const map = { COMMON: '1 dòng', UNCOMMON: '0 dòng', RARE: '2 dòng', EPIC: '3 dòng', LEGENDARY: '4 dòng', MYTHIC: '5 dòng' };
    return map[rarity];
};

const calculatePrice = () => {
    // 1. Sync Main Stat từ UI vào Form
    itemForm.atkBonus = 0;
    itemForm.defBonus = 0;
    itemForm.hpBonus = 0;
    ...
    // 2. Base Score
    let score = (['MATERIAL', 'CONSUMABLE'].includes(itemForm.type)) ? 50 : 200;

    // 3. Stat Score
    score += (itemForm.atkBonus || 0);
    score += (itemForm.defBonus || 0);
    score += (itemForm.hpBonus || 0);
    score += (itemForm.speedBonus || 0) * 200; // Speed đắt

    // 4. Multiplier
    const multMap = { COMMON: 1, UNCOMMON: 2, RARE: 5, EPIC: 15, LEGENDARY: 50, MYTHIC: 200 };
    const mult = multMap[itemForm.rarity] || 1;

    suggestedPrice.value = score * mult;
    itemForm.basePrice = suggestedPrice.value;
};

const createItem = async () => {
    try {
        calculatePrice(); // Final sync
        if (!itemForm.slotType) itemForm.slotType = itemForm.type;
...
const handleGrantEcho = async () => {
    try { await adminStore.grantEcho(grantEchoForm.username, grantEchoForm.amount); notificationStore.showToast("Đã ban EchoCoin!", "success"); grantEchoForm.username = ""; grantEchoForm.amount = ""; } catch(e) { notificationStore.showToast("Lỗi ban thưởng Echo!", "error"); }
};
const handleGrantItem = async () => {
    try { await adminStore.grantItem(grantItemForm.username, grantItemForm.itemId, grantItemForm.quantity); notificationStore.showToast("Đã ban bảo vật!", "success"); } catch(e) { notificationStore.showToast("Lỗi!", "error"); }
};
...
const confirmBan = async () => {
    if(!banReason.value) return notificationStore.showToast("Cần lý do trừng phạt!", "warning");
    try { await adminStore.banUser(selectedUser.value.userId, banReason.value); notificationStore.showToast("Đã thi hành án!", "success"); showBanModal.value = false; } catch(e) { notificationStore.showToast("Lỗi ban!", "error"); }
};
const requestUnban = async (u) => {
    if(confirm(`Ân xá cho ${u.username}?`)) { try { await adminStore.unbanUser(u.userId); notificationStore.showToast("Đã ân xá!", "success"); } catch(e) { notificationStore.showToast("Lỗi!", "error"); } }
};
const requestDeleteUser = async (u) => {
    if(confirm(`Xóa vĩnh viễn ${u.username}?`)) { try { await axiosClient.delete(`/admin/user/${u.userId}`); adminStore.fetchUsers(); notificationStore.showToast("Đã xóa!", "success"); } catch(e) { notificationStore.showToast("Lỗi!", "error"); } }
};
const requestDeleteItem = async (i) => {
    if(confirm(`Hủy bỏ vật phẩm ${i.name}?`)) { try { await axiosClient.delete(`/admin/item/${i.itemId}`); adminStore.fetchItems(); notificationStore.showToast("Đã hủy!", "success"); } catch(e) { notificationStore.showToast("Lỗi!", "error"); } }
};
...
.preview-img { width: 64px; height: 64px; border: 1px solid #444; border-radius: 4px; background: #000; }
...
.search-group input:focus { border-color: #ffca28; box-shadow: 0 0 10px rgba(255, 202, 40, 0.2); }
select { background: #222; color: #fff; border: 1px solid #555; padding: 10px 15px; border-radius: 4px; outline: none; }
.simple-form input, .simple-form textarea, .grid-form input, .grid-form select, .stats-inputs input { width: 100%; padding: 12px; background: rgba(0,0,0,0.4); border: 1px solid #555; color: #fff; border-radius: 4px; box-sizing: border-box; margin-bottom: 10px; font-family: inherit; }
