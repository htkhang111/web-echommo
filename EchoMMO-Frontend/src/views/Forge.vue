...
const handleUpgrade = async () => {
  ...
  try {
    ...
    const msg =
      ...
      err.response?.data ||
      "Lỗi hệ thống hoặc thiếu nguyên liệu.";
    showResultModal("fail", msg);
    errorMessage.value = msg;
  } finally {
    isForging.value = false;
...
const handleEvolve = async () => {
  ...
  try {
    ...
    await Promise.all([inventoryStore.fetchInventory(), authStore.fetchProfile()]);
    ...
    showResultModal(
      "fail",
      err.response?.data?.message || err.response?.data || "Lỗi tiến hóa.",
    );
  } finally {
    isForging.value = false;
    await fetchInventory();
...
const handleMythicUpgrade = async () => {
  ...
  try {
    ...
    console.error(err);
    const msg =
      err.response?.data?.message || err.response?.data || "Thăng cấp thất bại.";
    showResultModal("fail", msg);
  } finally {
    isForging.value = false;
  }
...
// --- HELPERS CỐT LÕI (ĐÃ FIX) ---
const formatNumber = (n) => Number(n || 0).toLocaleString();

// [FIX] Cập nhật mapping để khớp với tên file thật
const getMaterialImage = (name) => {
  const map = {
    "Gỗ": "r_wood.png",         // Đã sửa r_go -> r_wood
    "Sắt": "r_silver_bar.png",
    "Bạch Kim": "r_mystrile_bar.png",
    "Kim Cương": "stone_1.png", // Dùng tạm hình stone_1 nếu chưa có kim cương
  };
  return map[name] || "r_wood.png"; // Fallback an toàn
};
...
// [FIX] Sử dụng hình ảnh dự phòng an toàn nếu vẫn lỗi
const handleImgError = (e) => {
  e.target.src = resolveItemImage("r_stone_3.png"); // Chắc chắn file này tồn tại
};
