            ...
            <transition name="fade">
              <div v-if="selectedItem" class="upgrade-info">
                ...
                <div class="cost-section">
                  ...
                  <div 
                    class="resource-row"
                    :class="{ 'not-enough': !canAffordMaterial }"
                  >
                    <img
                      :src="
                        resolveItemImage(getMaterialImage(upgradeCost.matName))
                    ...
                    />
                    <span>
                      {{ formatNumber(userMaterialCount) }}/{{ upgradeCost.matQty }} 
                      {{ upgradeCost.matName }}
                    </span>
                  </div>
                ...
                <div class="actions-group">
                  <button
                    ...
                    class="btn-forge"
                    @click="handleUpgrade"
                    :disabled="isForging || !canAffordGold || !canAffordMaterial"
                  >
                    <span v-if="!isForging">CƯỜNG HÓA</span>
                    <span v-else
                      ><i class="fas fa-spinner fa-spin"></i> ĐANG RÈN...</span
...
// [MỚI] Tìm số lượng nguyên liệu trong kho
const userMaterialCount = computed(() => {
  if (!inventoryStore.items || !upgradeCost.value.matName) return 0;
  
  // Tìm item trong kho có tên trùng với tên nguyên liệu yêu cầu
  const matItem = inventoryStore.items.find(
    (i) => i.item && i.item.name === upgradeCost.value.matName
  );
  
  return matItem ? matItem.quantity : 0;
});
...
// [MỚI] Kiểm tra đủ nguyên liệu không
const canAffordMaterial = computed(() => {
  return userMaterialCount.value >= upgradeCost.value.matQty;
});
...
const handleUpgrade = async () => {
  ...
  // [MỚI] Chặn nếu thiếu vàng hoặc nguyên liệu
  if (!canAffordGold.value) {
      errorMessage.value = "Đại hiệp không đủ Vàng!";
      return;
  }
  if (!canAffordMaterial.value) {
      errorMessage.value = `Thiếu nguyên liệu: Cần ${upgradeCost.value.matQty} ${upgradeCost.value.matName}!`;
      return;
  }
