          ...
          <div class="char-block">
            <div class="char-text">
              <span class="greet-txt"
                ><i class="fas fa-fist-raised"></i> Tỷ Võ Luận Đạo</span
              >
          ...
          <div class="main-col">
            <div class="hero-card-wide">
              ...
              <div class="action-area">
                ...
                <div
                  v-if="gameState === 'MATCH_FOUND'"
                  class="match-popup-embed"
                >
                  ...
                  <div class="popup-actions-row">
                    <button
                      class="wuxia-btn outline danger"
                      @click="declineMatch"
                    >
                      BỎ QUA
                    </button>
        ...
        <div v-else class="battle-arena-frame">
          ...
          <transition name="fade">
            <div v-if="showLog" class="floating-log-panel custom-scroll">
              ...
              <div class="log-content">
                <div
                  v-for="(log, i) in battleLogs.slice().reverse()"
                  :key="i"
                  class="log-line"
                >
                  » {{ log }}
          ...
          <div class="fighters-stage">
            <div class="fighter left-side player">
              <div class="sprite-container">
                <img
                  :src="mySprite"
                  class="char-img pixel-art"
                  :class="{ 'shake-hit': isMyHit }"
                />
              ...
              <div class="hud player-hud">
                ...
                <div class="hp-track">
                  ...
                  <div
                    class="hp-fill"
                    :style="{ width: percent(myHp, myMaxHp) + '%' }"
                  ></div>
                </div>
              </div>
            </div>
            ...
            <div class="fighter right-side enemy">
              <div class="hud enemy-hud">
                ...
                <div class="hp-track">
                  ...
                  <div
                    class="hp-fill"
                    :style="{ width: percent(enemyHp, enemyMaxHp) + '%' }"
                  ></div>
                </div>
              </div>
              <div class="sprite-container">
                <img
                  :src="enemySprite"
                  class="char-img pixel-art enemy-flip"
                  :class="{ 'shake-hit': isEnemyHit }"
                />
                <div class="shadow-oval"></div>
              </div>
            </div>
            ...
            <div v-if="isRevealing" class="clash-visual">
              <div
                class="move-card p1"
                :class="getAnimClass(displayedMyMove, displayedEnemyMove, true)"
              >
              ...
              <div
                class="move-card p2"
                :class="
                  getAnimClass(displayedMyMove, displayedEnemyMove, false)
                "
              >
          ...
          <div class="battle-controls">
            ...
            <div class="actions-section">
              <div
                v-if="battlePhase === 'RPS_WAIT' && !isRevealing"
                class="rps-grid-simple"
              >
                <button
                  class="square-btn rock"
                  @click="submitRps('ROCK')"
                  :disabled="isActionPending"
                >
                  <i class="fas fa-hand-rock"></i><span>BÚA</span>
                </button>
                <button
                  class="square-btn paper"
                  @click="submitRps('PAPER')"
                  :disabled="isActionPending"
                >
                  <i class="fas fa-hand-paper"></i><span>BAO</span>
                </button>
                <button
                  class="square-btn scissor"
                  @click="submitRps('SCISSORS')"
                  :disabled="isActionPending"
                >
...
const characterName = computed(
  () => authStore.character?.name || authStore.user?.username || "Đại Hiệp",
);
...
const checkIsPlayer1 = (data) => {
  const myCharId = Number(
    authStore.character?.charId || authStore.character?.id || 0,
  );
...
const cancelSearch = async () => {
  try {
    await axios.post(`${API_URL}/cancel`, {}, { headers: getHeaders() });
  } catch (e) {}
  resetToLobby();
};
...
// [FIX] Xóa dòng ignoredMatchIds.value.add để frontend tiếp tục lắng nghe sự kiện FINISHED
...
const startPolling = () => {
  ...
  pollId = setInterval(async () => {
    try {
      ...
      const data = res.data;
      if (!data) return;
      if (data.matchId && ignoredMatchIds.value.has(Number(data.matchId)))
        return;
      ...
      if (data.status === "FINISHED") {
        if (!showEndGameModal.value) {
          ...
          const serverWinnerId =
            data.winnerId !== null ? Number(data.winnerId) : null;
          ...
          setTimeout(() => {
            ...
            const isDisconnect = log.includes("thoát");
            ...
            if (serverWinnerId === null) {
              ...
              endGameMessage.value = isSurrender
                ? "Đối thủ đã đầu hàng!"
...
const syncBattleData = (data) => {
  ...
  if (nMyHp < myHp.value) {
    isMyHit.value = true;
    setTimeout(() => (isMyHit.value = false), 300);
  }
  if (nEnHp < enemyHp.value) {
  ...
  if (data.messages && data.messages.length > matchMessages.value.length) {
    data.messages.slice(matchMessages.value.length).forEach((m) =>
      matchMessages.value.push({
        sender: m.senderName,
        text: m.content,
        isMe: (checkIsPlayer1(data) ? data.p1Id : data.p2Id) == m.senderId,
      }),
    );
    nextTick(() => {
      if (chatBoxRef.value)
  ...
  if (bothMoved && currentTurn > lastProcessedTurn.value) {
    ...
    if (data.lastLog) {
      ...
      if (logUpper.includes("NÉ ĐƯỢC") || logUpper.includes("NÉ ĐÒN")) {
        lastResultText.value = "NÉ ĐÒN!";
      } else if (logUpper.includes("CHÍ MẠNG")) {
        lastResultText.value = "BẠO KÍCH!";
      } else if (logUpper.includes("THẮNG")) {
        lastResultText.value = "TRÚNG!";
      } else if (logUpper.includes("THUA")) {
        lastResultText.value = "TRÚNG!";
      } else {
        lastResultText.value = "XUNG KHẮC!";
      }
    }
...
const sendPrivateChat = async () => {
  ...
  try {
    await axios.post(
      `${API_URL}/chat`,
      { matchId: matchId.value, message: chatInput.value },
      { headers: getHeaders() },
    );
  } catch (e) {}
  chatInput.value = "";
...
.avatar-frame.large {
  ...
  border-radius: 50%;
  overflow: visible;
  background: radial-gradient(circle, #261815 60%, #000 100%);
...
.fighter.left-side .hud {
  text-align: left;
}
.fighter.right-side {
  align-items: flex-end;
}
.fighter.right-side .hud {
  text-align: right;
}
...
.res-crit {
  color: #ff5252;
  text-shadow: 0 0 15px #d50000;
  transform: scale(1.2);
  animation: critShake 0.2s infinite;
}
.res-clash {
  color: #bcaaa4;
}
...
const manualRefresh = async () => {
    // Có thể thêm logic reload leaderboard hoặc reset state nếu cần
...
const startPolling = () => {
  ...
  pollId = setInterval(async () => {
    try {
      ...
      // [1] XỬ LÝ KẾT THÚC TRẬN ĐẤU
      if (data.status === "FINISHED") {
        // [FIXED] CHẶN TRẬN ĐẤU CŨ KHI VỪA VÀO TRANG HOẶC ĐANG TÌM TRẬN
        if (gameState.value === 'LOBBY' || gameState.value === 'SEARCHING') {
            if (data.matchId) {
                ignoredMatchIds.value.add(Number(data.matchId));
            }
            return; // Bỏ qua ngay
        }
