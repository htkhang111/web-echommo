          ...
          <div class="battle-controls">
            ...
            <div class="actions-section">
              ...
              <div v-else class="status-overlay">
                ...
                <button v-if="!isRevealing" class="text-btn-tiny" @click="handleSurrender">Thoát trận</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <transition name="fade">
        <div v-if="showEndGameModal" class="endgame-modal-overlay">
            <div class="endgame-content" :class="{'win': isWin, 'lose': !isWin}">
                ...
                <div class="auto-back-timer">Tự động về sảnh sau {{ autoBackTimer }}s</div>
                <button class="wuxia-btn primary big" @click="closeEndGameModal">VỀ SẢNH NGAY</button>
            </div>
        </div>
      </transition>
  </div>
  ...
  pollId = setInterval(async () => {
    try {
      ...
      if (data.status === 'FINISHED') {
         // Nếu chưa hiện modal thì xử lý kết thúc
         if (!showEndGameModal.value) {
             const myCharId = Number(authStore.character?.charId || authStore.character?.id || 0);
             const serverWinnerId = data.winnerId !== null ? Number(data.winnerId) : null;
             
             // Cập nhật Battle Data lần cuối để hiện máu = 0 hoặc move cuối
             syncBattleData(data); 

             // Nếu đang có animation (isRevealing), chờ nó xong (3s) rồi mới hiện bảng kết quả
             ...
             setTimeout(() => {
                 ...
                 if (serverWinnerId === null) {
                     ...
                     endGameMessage.value = isSurrender ? "Bạn đã đầu hàng!" : "Bạn đã bị đánh bại...";
                 }

                 showEndGameModal.value = true;
                 
                 // [AUTO LOBBY] Tự động về sảnh sau 5s
...
.fighter.left-side { align-items: flex-start; }
.fighter.left-side .hud { text-align: left; }
.fighter.right-side { align-items: flex-end; }
.fighter.right-side .hud { text-align: right; }
