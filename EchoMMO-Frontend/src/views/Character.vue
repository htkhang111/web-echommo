...
// --- 2. CẤU HÌNH SLOT (ĐÃ CẬP NHẬT GATHERING TOOLS) ---
const SLOT_CONFIG = {
  // Trang bị chiến đấu
  NECKLACE: { label: "Dây Chuyền", icon: "fa-gem", className: "necklace-slot" },
  WEAPON:   { label: "Vũ Khí",     icon: "fa-gavel", className: "weapon-slot" },
  ...
  // Công cụ khai thác (Mới)
  PICKAXE:     { label: "Cúp",     icon: "fa-hammer",       className: "tool-pickaxe" },
  AXE:         { label: "Rìu",     icon: "fa-axe",          className: "tool-axe" }, // Cần import fontawesome có icon này hoặc dùng icon thay thế
  SHOVEL:      { label: "Xẻng",    icon: "fa-trowel",       className: "tool-shovel" },
  FISHING_ROD: { label: "Cần Câu", icon: "fa-fish",         className: "tool-fishing" },
};
...
const bagItems = computed(() => {
  ...
  return items.filter(ui => {
    ...
    // Logic cũ: Lọc bỏ Material/Consumable để không hiện trong tab trang bị
    // Nhưng TOOLS (Cúp, Rìu...) thì VẪN HIỆN để người chơi bấm Equip
    const type = (ui.item.type || "").toUpperCase();
    if (type === 'MATERIAL' || type === 'RESOURCE') { // Giữ Consumable nếu muốn
        return false;
    }
    // Chỉ hiện những món có slot hợp lệ (Trang bị + Tool)
    return determineSlot(ui.item) !== null;
  });
});
...
.char-center-col {
    ...
    height: 100%;
    display: flex;
    flex-direction: column; /* Để xếp toolbelt xuống dưới */
    align-items: center;
    justify-content: center;
}
...
.tool-belt-title {
    font-size: 10px; color: #a1887f; text-align: center; margin-bottom: 5px; letter-spacing: 1px;
}
.tool-row {
...
.mini-tool-slot i { color: #5d4037; font-size: 1.2rem; }
...
.equip-slot.filled .slot-frame { border-color: var(--gold); background: rgba(255,236,179,0.05); box-shadow: 0 0 15px rgba(255,215,0,0.3); }
.equip-slot.target-glow .slot-frame, .mini-tool-slot.target-glow { border-color: #66bb6a; box-shadow: 0 0 20px #66bb6a; transform: scale(1.05); }
