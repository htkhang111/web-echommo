...
.fade-slide-enter-active, .fade-slide-leave-active { transition: all 0.3s ease; } .fade-slide-enter-from, .fade-slide-leave-to { opacity: 0; transform: translateY(10px); }
      ...
      await marketStore.sellItem(data.id, data.qty);
      const idx = inventoryStore.items.findIndex(
        (i) => i.userItemId === data.id,
      );
      if (idx !== -1 && inventoryStore.items[idx].quantity <= data.qty) {
        inventoryStore.items.splice(idx, 1);
      }
      notificationStore.showToast(
        `Đã bán, nhận ${formatNumber(data.total)} xu!`,
        "success",
      );
    }
    authStore.fetchProfile();
    ...
    if (type === "P2P") await marketStore.fetchPlayerListings();
    const msg =
      e.response?.data?.message || "Giao dịch thất bại, vui lòng thử lại.";
    notificationStore.showToast(msg, "error");
  }
};
...
const handleCancelListing = async (listingId) => {
  ...
  try {
    ...
    notificationStore.showToast("Lỗi thu hồi.", "error");
  }
};
...
:root {
  ...
  /* Màu phẩm chất - Update theo tên Backend */
  --r-common-color: #a0a0a0;
  --r-uncommon-color: #4caf50;
  --r-rare-color: #00b0ff;
  --r-epic-color: #d500f9;
...
/* =========================================
   RARITY STYLES (MATCHING BACKEND NAMES)
   ========================================= */

/* COMMON (Phàm) */
.border-COMMON {
  border: 2px solid var(--r-common-color);
  box-shadow: 0 0 2px var(--r-common-color);
}
.bg-COMMON {
  background: linear-gradient(135deg, #424242, #616161);
  color: #fff;
  border: 1px solid var(--r-common-color);
}
.text-COMMON {
...
/* UNCOMMON (Lương) */
.border-UNCOMMON {
  border: 2px solid var(--r-uncommon-color);
...
.text-UNCOMMON {
  color: var(--r-uncommon-color);
  text-shadow: 0 0 5px rgba(76, 175, 80, 0.4);
}

/* RARE (Linh) */
.border-RARE {
  border: 2px solid var(--r-rare-color);
  box-shadow:
    0 0 8px rgba(0, 176, 255, 0.4),
    inset 0 0 5px rgba(0, 176, 255, 0.2);
}
.bg-RARE {
  background: linear-gradient(135deg, #01579b, #0288d1);
  color: #fff;
  box-shadow: 0 0 5px var(--r-rare-color);
  border: 1px solid #81d4fa;
}
.text-RARE {
  color: var(--r-rare-color);
  text-shadow: 0 0 5px rgba(0, 176, 255, 0.5);
}

/* EPIC (Tiên) */
.border-EPIC {
  border: 2px solid var(--r-epic-color);
  box-shadow:
    0 0 10px rgba(213, 0, 249, 0.5),
    inset 0 0 8px rgba(213, 0, 249, 0.3);
}
.bg-EPIC {
  background: linear-gradient(135deg, #4a148c, #7b1fa2);
  color: #fff;
  box-shadow: 0 0 8px var(--r-epic-color);
...
/* LEGENDARY (Thần) */
.border-LEGENDARY {
  border: 2px solid var(--r-legendary-color);
  box-shadow:
    0 0 10px var(--r-legendary-color),
...
.bg-LEGENDARY {
  ...
  box-shadow: 0 0 10px var(--r-legendary-color);
}
.text-LEGENDARY {
  color: var(--r-legendary-color);
  text-shadow:
...
/* MYTHIC (Thánh) */
.border-MYTHIC {
  border: 2px solid var(--r-mythic-color);
  box-shadow:
    0 0 15px var(--r-mythic-color),
...
.bg-MYTHIC {
  ...
  box-shadow: 0 0 10px var(--r-mythic-color);
}
.text-MYTHIC {
  color: var(--r-mythic-color);
  text-shadow:
...
@keyframes pulse-gold {
  ...
  100% {
    box-shadow:
      0 0 20px var(--r-legendary-color),
