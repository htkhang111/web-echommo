      ...
      <div class="content-layout">
        <div class="inventory-panel glass-panel">
          ...
          <div class="item-grid custom-scroll">
            ...
            >
              <img :src="resolveItemImage(item.item.image || item.item.imageUrl)" @error="handleImgError">
          ...
          <div v-if="selectedItem" class="evolution-active">
            <div class="item-display">
              <div class="item-box glowing-border">
                 <img :src="resolveItemImage(selectedItem.item.image || selectedItem.item.imageUrl)" class="main-img">
              </div>
              <h3 class="item-name text-legendary">{{ selectedItem.item.name }}</h3>
            </div>
    ...
    <transition name="fade">
      <div v-if="showResult" class="result-overlay" @click="closeResult">
        <div class="result-box">
          ...
          <p>Chúc mừng! {{ selectedItem?.item.name }} đã đạt cảnh giới Thần Thoại.</p>
...
const handleEvolve = async () => {
    ...
    try {
        await axiosClient.post(`/equipment/evolve-mythic/${selectedItem.value.userItemId}`);
...
.inventory-panel { flex: 1; max-width: 350px; }
.evolution-panel { flex: 2; align-items: center; justify-content: center; }
...
.item-slot.selected { border-color: #ff00cc; box-shadow: 0 0 10px #ff00cc; }
...
.main-img { width: 100px; height: 100px; object-fit: contain; }

.text-legendary { color: #ffd700; text-shadow: 0 0 5px #ffaa00; margin-top: 10px;}

.arrow-down { color: #ff00cc; font-size: 2rem; animation: bounce 2s infinite; }
...
.mythic-label { font-weight: bold; color: #ff00cc; letter-spacing: 2px; display: block; }
.boost-val { color: #00ff00; }
...
.cost-item.missing { border-color: #ff3333; color: #ff3333; }
.icon-sm { width: 24px; height: 24px; }
.text-gold { color: #ffd700; }
.current-qty { margin-left: auto; font-size: 0.8rem; opacity: 0.7; }
...
.btn-evolve:disabled { filter: grayscale(1); cursor: not-allowed; }
.btn-evolve:hover:not(:disabled) { box-shadow: 0 0 30px #ff0066; }
...
.result-box {
    background: #1a051a; border: 2px solid #ff00cc;
    padding: 40px; text-align: center; border-radius: 10px;
    box-shadow: 0 0 50px #ff00cc;
    animation: zoomIn 0.3s ease;
}
.mythic-icon-box { font-size: 4rem; color: #ff00cc; margin: 20px 0; }
.btn-confirm {
    background: transparent; border: 1px solid white; color: white;
    padding: 10px 30px; cursor: pointer; margin-top: 20px;
}
