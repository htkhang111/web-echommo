...
public class EquipmentService {
    ...
    public UserItem enhanceMythicStars(Long userItemId, Integer userId) {
        ...
        if (random.nextInt(100) < successRate) {
            item.setMythicStars(nextStar);
            BigDecimal currentVal = item.getMainStatValue();
            if (currentVal == null) currentVal = BigDecimal.ZERO; // [Safe check]
    ...
    public UserItem enhanceItem(Long userItemId) {
        ...
        // [FIX] Nếu baseVal vẫn null (do data cũ/lỗi), gán bằng 0 để tránh lỗi NullPointerException
        if (baseVal == null) {
            baseVal = BigDecimal.ZERO;
        }

        // Nếu lần đầu cường hóa, lưu lại giá trị gốc vào originalMainStatValue để tham chiếu sau này
